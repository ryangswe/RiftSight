...
`PORT` is injected automatically by Railway — do not set it yourself; `relay/src/env.ts` already prefers `PORT` over `RELAY_PORT` when both are present, matching Railway's convention. `TWITCH_EXTENSION_CLIENT_ID` and `ALLOW_LOCAL_DEBUG` are optional (see README's env var table) and can be left unset in closed-beta.
...
- **`RIFTSIGHT_BACKEND_URL`** (build-time env var for `extension/`): `https://<your-domain>` — rebuild with `RIFTSIGHT_MODE=closed-beta RIFTSIGHT_BACKEND_URL=https://<your-domain> npm run build -w extension`.
...
## 7. Migrations run at startup — never add a `preDeployCommand` for this

`relay/src/index.ts` already applies any pending migration automatically on every boot (`runMigrations()` is idempotent — a no-op once the database is current), which is what makes it safe to run this way at all: see the operator runbook's migration notes. `railway.json` deliberately has no `deploy.preDeployCommand` set, and none should be added for migrations specifically, for a concrete reason beyond just redundancy: `preDeployCommand` runs in a separate, ephemeral pre-deploy step, and it isn't guaranteed to have the same persistent volume mounted that the real running service does (the volume can only be attached to one active mount at a time, and a pre-deploy step is exactly the kind of transient, parallel-to-the-old-deployment execution that could either fail to get it or — worse — silently succeed against an empty ephemeral filesystem, creating a throwaway SQLite file that gives false confidence a migration "worked" when the real database was never touched). Running migrations inside the actual service's own startup, as this app already does, sidesteps that ambiguity entirely: the code only ever runs once it's the real process with the real volume mounted.

## 8. Stay at exactly one replica

`railway.json` sets `numReplicas: 1` and `sleepApplication: false` — do not raise the replica count, enable Railway's autoscaling, or turn on the "Serverless"/app-sleeping toggle for this service (Settings → Deploy → Serverless is off by default on Railway's own Hobby-plan default, but is easy to enable later without realizing the consequences below — `sleepApplication: false` in `railway.json` is a config-level guarantee against that, not just a reminder not to click it). Two concrete reasons this has to stay single-instance, both already true of this architecture regardless of platform (see README's "Known limitations"):

- **Local SQLite**: `RIFTSIGHT_DB_PATH=file:/data/riftsight.db` is a single file on the one volume mounted to this one replica. A second replica would either fail to start (can't get an exclusive lock on the same file from two processes) or, worse, silently corrupt writes if Railway's volume semantics ever allowed two containers to see the same path without proper locking — this is not a supported configuration, not something that's been tested to fail gracefully.
- **In-memory live session state**: `relay/src/server.ts`'s `Session` map (which producer is connected, which viewers are subscribed, the latest overlay state) lives entirely in one process's memory. A second replica would have a completely independent, empty set of sessions — a producer connecting to replica A and a viewer connecting to replica B would simply never see each other, silently, with no error on either side. Sleeping and waking the service has a similar effect for the single replica that exists: every connected producer/viewer WebSocket is dropped and has to reconnect, which for a live stream is a visible interruption, not just an idle-cost optimization gone unnoticed.

Migrations are also **not** designed for concurrent execution from multiple instances racing to apply the same migration — another reason this stays single-replica for now.
