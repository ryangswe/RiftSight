...
1. **Remote database**: provision Turso, run migrations against it once (`RIFTSIGHT_DB_PATH=libsql://… TURSO_AUTH_TOKEN=… npm run migrate -w relay`), copy the current allowlist/broadcaster/credential rows over with `npm run copy-db -w relay` (ids preserved, counts verified — see the operator runbook's "Remote database (Turso) cutover"), and point the service's `RIFTSIGHT_DB_PATH` at the `libsql://…` URL with `TURSO_AUTH_TOKEN` set alongside it. The local file and its volume stop being used.
...
Do the Stage-2 staging spike first (WS load-balancing/draining behavior — see the scaling plan's operator checklist) before touching the production service: a second Railway service from this repo with its config-file path set to `railway.staging.json` (2 replicas, no volume mount, otherwise identical) and its own Turso/Redis.
