...
- A **reference screenshot**: drop in an image of your actual stream output (file picker, drag-and-drop onto the preview, or paste from the clipboard) to see it behind the region box, so you can align by eye against your real composition instead of guessing blind. Loaded via `URL.createObjectURL` — stays entirely in the browser tab, never uploaded or persisted, and is gone on the next page load. If RiftAtlas fills the whole canvas, the default **Full frame** preset needs no screenshot at all.
...
- Numeric **x / y / width / height** inputs as an advanced/exact-values fallback (always available and authoritative — the source of truth even if you never touch the visual preview or a screenshot). An invalid value (e.g. one that would push the region outside the frame) reverts to the last valid one rather than saving something broken.
